...
<body>
	<form:form id="searchForm" modelAttribute="SExam" action="${ctx}/studentExam/homeWorkList" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<input name="examType" type="hidden" value="3" >
		<ul class="ul-form">
			<li><label>名称：</label>
				...
				<tr>
					...
					<td>
						...
						<c:if test="${exam.examStatus == 3 }">
							<%-- <a target="_black" href="${ctx}/studentAnswer/form?examId=${exam.id }&returnUrl=/studentExam/homeWorkList&examType=3">进入</a> --%>
							 
							<a target="_top"  href="${ctx}/studentAnswer/form?examId=${exam.id }&returnUrl=/studentExam/homeWorkList&examType=3" style="background: url(/static/icon/jr.png) no-repeat;width:20px;height:20px;display:inline-block;background-size:100%;cursor:pointer;" title="进入"></a>
						</c:if>
						<c:if test="${exam.examStatus == 1 }">
							<a href="${ctx}/studentExam/searchExamQuestions?examId=${exam.id }&examType=3&&examDetailId=${exam.examDetailId}" style="background: url(${ctxStatic}/icon/check.png) no-repeat;width:20px;height:20px;background-size:100%;cursor:pointer;display:inline-block;" title="查看"></a>
						</c:if>
					</td>
				</tr>
			</c:forEach>
