	...
	<script type="text/javascript">
	   ...
	   $("#contentTable").treeTable({expandLevel : 2}).show();
		...
		function saveButton(){ 
			...
			window.location.href ='${ctx}/examination/adjustExam?examId='+examId; 
		}
	</script>
</head>
<body>

     <form:form id="searchForm"  action="${ctx}/examination/adjustExamList?examdetailId=${examdetailId}&examId=${examId}" method="post" class="breadcrumb form-search ">
		...
		<label>出题时间：</label>
		  <input id="beginTime" name="beginTime"     type="text"  readonly="readonly" maxlength="20"  class="input-mini Wdate"
		         ...
		         onclick="WdatePicker({dateFmt:'yyyy-MM-dd',autoPickDate:true,isShowClear:true,maxDate:'new date'});" readonly/>
		  ~<input id="endTime" name="endTime" type="text" readonly="readonly" maxlength="20" class="input-mini Wdate"
				value="<fmt:formatDate value="${endTime}" pattern="yyyy-MM-dd"/>" 
				onclick="WdatePicker({dateFmt:'yyyy-MM-dd',autoPickDate:true,isShowClear:true, minDate:'#F{$dp.$D(\'beginTime\')}',maxDate: 'new date'})" readonly/>
		<label>出题类型：</label>
		...
		<input id="btnSubmit1" class="btn tbtn-primary" type="submit" value="查询"/>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed"  style="margin-top: 20px;">
		<thead>
			<tr>
				...
				<th style="width:3%;">题型</th>
				<th style="width:70%;">题干</th>
				<th style="width:5%;">出题频率</th>
		...
		<tbody>
		 <c:forEach items="${dataMap}" var="itemKey" varStatus="index">
			...
			<tr  id="${item.question.id }" pId="${itemKey.key}" >
				...
				<td style="width:70%;">${item.question.title}</td>
				<td>
				    ${item.count}
				</td>
				 <td>
		 ...
		 <%-- <c:forEach items="${dataMapValues}" var="item" varStatus="index">
			<tr>
				<td>${(index.index+1)}</td>
	...
	<script type="text/javascript">
		(function(){
			$("#contentTable tbody tr").find("td:last-child a").click(function(){
				  ...
				  var examId='${examId}';
				  var quesType=$(this).parent().parent().attr("pId");
				  top.$.jBox("iframe:${ctx}/examination/adjustExamInfo?questionId="+id+"&examdetailId="+examdetailId+"&examId="+examId+"&questype="+quesType,{
						title:"替换题目",
						width: 900,
					    height: 350,
						buttons:{}, 
					    ...
					    loaded:function(h){
							...
							if(b>0){
								...
								window.location.href ='${ctx}/examination/adjustExamList?examdetailId='+examdetailId+"&examId="+examId; 
								return false;
							}
						},closed:function(){
							/* $("#searchForm").submit(); */
