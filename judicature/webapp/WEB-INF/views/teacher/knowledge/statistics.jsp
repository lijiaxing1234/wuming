			...
			(function(){
				var setting = { data:{simpleData:{enable:true,idKey:"id",pIdKey:"pId",rootPId:'0'},view:{showIcon:false}},
					callback:{ onClick:function(event, treeId, treeNode){
							var id = treeNode.id;
							$('#knowledgeContent').attr("src","${ctx}/knowledge/statisticsList2?courseKnowledge.id="+id);
						}
					}
				};
			 ...
			 (function(){
				...
				function wSize(){
					...
					console.log(mainRight.height());
					
					frameObj.height(mainRight.height()- 5);
				    $("#left").width("180").css({"float":"left"});
					var leftWidth = ($("#left").width() < 0 ? 0 : $("#left").width());
					$("#right").width(mainRight.width()- leftWidth -20).css({"float":"left"});
