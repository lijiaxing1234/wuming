		...
		function page(n,s){
		 ...
		 $(document).ready(function() {
			 ...
			 $("input[id^='Example_']").click(function(){
				...
				window.location.href = ("${ctx}/examination/adjustExam?id="+examId+"&status="+status);
			 });
			 /* 查询 */
			 $("input[id^='ExamQuerySubmit_']").click(function(){
				 var examId=$(this).attr("id").split("_")[1]; 
				 var examStatus=$(this).attr("id").split("_")[3];
				window.location.href = ("${ctx}/testPaper/testPaperUpExam?examId="+examId+"&examStatus="+examStatus);
			 });
			 /* 发布考试 */
			 ...
			 $("input[id^='ExamSaveSubmit_']").click(function(){
				 var examId=$(this).attr("id").split("_")[1]; 
				 var status=$(this).attr("id").split("_")[3]; 
				window.location.href = ("${ctx}/testPaper/saveExam?examId="+examId+"&status="+status+"&examStatus=no");
				alertx("保存成功！");
			 });
			 /* 删除 */
			 $("input[id^='ExamDelSubmit_']").click(function(){
				 ...
				 return confirmx("确认要删除当前试卷吗？",function(){
					 window.location.href = ("${ctx}/testPaper/deleteTestPaper?examId="+examId+"&examClassId="+examClassId+"&status="+status+"&examStatus=no");
				});
			 });
			//判卷
...
<body>
 <ul class="nav nav-tabs">
	<li class="active"><a href="${ctx}/testPaper/testPaperListUnSubmit">未结束的考试</a></li>
	<li><a href="${ctx}/testPaper/testPaperList">已结束的考试</a></li>
</ul>
<sys:message content="${message}"/>
    <form:form id="testpaperForm" modelAttribute="exam" action="${ctx}/testPaper/testPaperListUnSubmit" method="post" class="breadcrumb form-search ">
					...
					<td style="text-align: center;">
						...
						<!-- 未存为模板的试卷才能存为模板 -->
							<c:if test="${list.istemplate eq '0'}">
								<input id="ExamSaveSubmit_${list.id}_${list.examClass.id}_no" style="background: url(${ctxStatic}/icon/mb.png) no-repeat;width:20px;height:20px;display:inline-block;background-size:100%;cursor:pointer;"  type="button" title="存为模板" />
							...
							<%-- <c:if test="${list.examType eq '5'}">
								...
								<c:if test="${list.status eq '2'}">
									<c:choose>
										...
										<c:otherwise>
											<input id="ExamIsMark_${list.id}_${list.examClass.id}_no" class="btn btn-primary" type="button" value="判卷" />
										</c:otherwise>
									</c:choose>
									<input id="ExamQuerySubmit_${list.id}_${list.examClass.id}_no" class="btn btn-primary" type="button" value="查看试卷" />
								</c:if>
							</c:if> --%>
							<!-- 组卷考试 -->
							<c:if test="${list.examType eq '2'}">
								<c:if test="${list.status eq '0'}">
									<input id="Example_${list.id}_${list.examClass.id}_no" style="background: url(${ctxStatic}/icon/xg.png) no-repeat;width:20px;height:20px;display:inline-block;background-size:100%;cursor:pointer;"  type="button" title="修改" />
									<input id="ExamDelSubmit_${list.id}_${list.examClass.id}_no"  style="background: url(${ctxStatic}/icon/del.png) no-repeat;width:20px;height:20px;display:inline-block;background-size:100%;cursor:pointer;"  type="button" title="删除" />
								</c:if>
								<c:if test="${list.status eq '1'}">
									<input id="ExamQuerySubmit_${list.id}_${list.examClass.id}_no" style="background: url(${ctxStatic}/icon/check.png) no-repeat;width:20px;height:20px;display:inline-block;background-size:100%;cursor:pointer;"  type="button" title="查看试卷" />
								</c:if>
								<c:if test="${list.status eq '2'}">
									<c:choose>
										<c:when test="${list.isTeacher eq '1'}">
											...
											<input id="examSubmit_${list.examClass.id}_${list.id}_no" style="background: url(${ctxStatic}/icon/check.png) no-repeat;width:20px;height:20px;display:inline-block;background-size:100%;cursor:pointer;"  type="button" title="查看成绩"/>
										</c:when>
										<c:otherwise>
											<input id="saveSubmit_${list.examClass.id}_${list.id}_no"  style="background: url(${ctxStatic}/icon/xg.png) no-repeat;width:20px;height:20px;display:inline-block;background-size:100%;cursor:pointer;"  type="button" title="录入成绩" />
											<input id="saveSubmit_${list.examClass.id}_${list.id}_no" style="background: url(${ctxStatic}/icon/lr.png) no-repeat;width:20px;height:20px;display:inline-block;background-size:100%;cursor:pointer;"  type="button" title="修改成绩" />
										</c:otherwise>
									</c:choose>
									<input id="ExamQuerySubmit_${list.id}_${list.examClass.id}_no" style="background: url(${ctxStatic}/icon/check_1.png) no-repeat;width:20px;height:20px;display:inline-block;background-size:100%;cursor:pointer;"  type="button" title="查看试卷" />
								</c:if>
							</c:if>
						</c:if>
						<!-- 自动 -->
							...
							<c:if test="${list.examType eq '2'}">
								...
								<c:if test="${list.status eq '2'}">
									<c:choose>
										...
										<c:otherwise>
											<input id="saveSubmit_${list.examClass.id}_${list.id}_no" style="background: url(${ctxStatic}/icon/lr.png) no-repeat;width:20px;height:20px;display:inline-block;background-size:100%;cursor:pointer;"  type="button" title="录入成绩" />
											<input id="saveSubmit_${list.examClass.id}_${list.id}_no" style="background: url(${ctxStatic}/icon/xg.png) no-repeat;width:20px;height:20px;display:inline-block;background-size:100%;cursor:pointer;"  type="button" title="修改成绩" />
										</c:otherwise>
									</c:choose>
									<input id="ExamQuerySubmit_${list.id}_${list.examClass.id}_no"  style="background: url(${ctxStatic}/icon/check_1.png) no-repeat;width:20px;height:20px;display:inline-block;background-size:100%;cursor:pointer;"  type="button" title="查看试卷" />
								</c:if>
