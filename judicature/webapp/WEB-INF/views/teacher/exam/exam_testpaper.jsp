		...
		function page(n,s){
		 ...
		 $(document).ready(function() {
			 ...
			 $("input[id^='ExamSaveSubmit_']").click(function(){
				 var examId=$(this).attr("id").split("_")[1]; 
				 var status=$(this).attr("id").split("_")[3]; 
				window.location.href = ("${ctx}/testPaper/saveExam?examId="+examId+"&status="+status);
				alertx("保存成功！");
			 });
			 /* 删除 */
			 $("input[id^='ExamDelSubmit_']").click(function(){
				 ...
				 return confirmx("确认要删除当前试卷吗？",function(){
					 window.location.href = ("${ctx}/testPaper/deleteTestPaper?examId="+examId+"&examClassId="+examClassId+"&status="+status);
				});
			 });
			//判卷
...
<body>
 <ul class="nav nav-tabs">
	<li><a href="${ctx}/testPaper/testPaperListUnSubmit">未结束的考试</a></li>
	<li class="active"><a href="${ctx}/testPaper/testPaperList">已结束的考试</a></li>
</ul>
<sys:message content="${message}"/>
    <form:form id="testpaperForm" modelAttribute="exam" action="${ctx}/testPaper/testPaperList" method="post" class="breadcrumb form-search ">
				...
				<tr>
					...
					<td style="text-align: center;">
						...
						<c:if test="${list.examMode eq '0'}">
							...
							<c:if test="${list.examType eq '2'}">
								<c:if test="${list.status eq '0'}">
									<input id="Example_${list.id}_${list.examClass.id}_no" class="btn tbtn-primary" type="button" value="修改"" />
									<input id="ExamDelSubmit_${list.id}_${list.examClass.id}_no" class="btn tbtn-primary" type="button" value="删除" />
								</c:if>
								<c:if test="${list.status eq '1'}">
									<input id="ExamQuerySubmit_${list.id}_${list.examClass.id}_no" class="btn tbtn-primary" type="button" value="查看试卷" />
								</c:if>
								<c:if test="${list.status eq '2'}">
									<c:choose>
										<c:when test="${list.isTeacher eq '1'}">
