		...
		function page(n,s){
		 ...
		 $(document).ready(function() {
			 ...
			 $("input[id^='ExamSaveSubmit_']").click(function(){
				 var examId=$(this).attr("id").split("_")[1]; 
				window.location.href = ("${ctx}/testPaper/saveExam?examId="+examId+"&examStatus=no");
				alertx("保存成功！");
			 });
			 /* 删除 */
...
<body>
 <ul class="nav nav-tabs">
	<li class="active"><a href="${ctx}/testPaper/testPaperOnLineListUnSubmit">未结束的考试</a></li>
	<li><a href="${ctx}/testPaper/testPaperOnLineList">已结束的考试</a></li>
</ul>
<sys:message content="${message}"/>
    <form:form id="testpaperForm" modelAttribute="exam" action="${ctx}/testPaper/testPaperOnLineListUnSubmit" method="post" class="breadcrumb form-search ">
					...
					<td style="text-align: center;">
						...
						<!-- 未存为模板的试卷才能存为模板 -->
							<c:if test="${list.istemplate eq '0'}">
								<input id="ExamSaveSubmit_${list.id}_${list.examClass.id}" class="btn tbtn-primary" type="button" value="存为模板" />
							</c:if>
							<!-- 在线考试才有发布考试的功能 -->
							<c:if test="${list.examType eq '5'}">
								<!-- 只有状态为未发布的才有发布功能   0未发布 1正在进行 2已完成 -->
...
<%-- 									<input id="ExamUpdateSubmit_${list.id}_${list.examClass.id}" class="btn tbtn-primary" type="button" value="发布" /> --%>		
									<input id="ExamDelSubmit_${list.id}_${list.examClass.id}" class="btn tbtn-primary" type="button" value="删除" />
								</c:if>
								<!-- 已完成 -->
								<c:if test="${list.status eq '2'}">
									<c:choose>
									...
									<input id="ExamQuerySubmit_${list.id}_${list.examClass.id}_online"   style="background: url(${ctxStatic}/icon/check.png) no-repeat;width:20px;height:20px;display:inline-block;background-size:100%;cursor:pointer;"  type="button" title="查看试卷" />
								</c:if>
							</c:if>
							<!-- 组卷考试 -->
							<%-- <c:if test="${list.examType eq '2'}">
								<c:if test="${list.status eq '0'}">
									<input id="Example_${list.id}_${list.examClass.id}" class="btn tbtn-primary" type="button" value="修改" />
									<input id="ExamDelSubmit_${list.id}_${list.examClass.id}" class="btn tbtn-primary" type="button" value="删除" />
								</c:if>
								<c:if test="${list.status eq '2'}">
									<c:choose>
...
<%-- 									<input id="ExamUpdateSubmit_${list.id}_${list.examClass.id}"  style="background: url(${ctxStatic}/icon/fb.png) no-repeat;width:20px;height:20px;display:inline-block;background-size:100%;cursor:pointer;"  type="button" title="发布" />
