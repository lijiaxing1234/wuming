...
<html>
	...
	<script type="text/javascript">
		...
		function importQuestions(){
			window.location.href="${ctx}/school/schoolversionQuestion/importDoc?extId=${extId}&courseQuestionlibId=${courseQuestionlibId}&courseVersionId=${courseVersionId}";
		}
		...
		function emportQuestions(){
			...
			//alert(delFlag);
			$.ajax({
		         url:"${ctx}/school/schoolversionQuestion/emportQuestionWord?courseQuestionlibId="+courseQuestionlibId+"&extId="+extId+"&delFlag="+delFlag,  
		         type:"post",
		         cache:false,
		...
		(function(){
		    $(function(){
		    	...
		    	//	alert(courseQuestionlibId);
		    		openJbox("iframe:${ctx}/school/schoolversionQuestion/importQuestionWordForm?courseQuestionlibId="+courseQuestionlibId,"试题导入");
		    	...
		    	$("#emportButton").on("click",function(){
		    		...
		    		//alert(delFlag);
		    		var courseQuestionlibId = $("#questionlibId").val();
		    		openJbox("iframe:${ctx}/school/schoolversionQuestion/emportQuestionWordForm?courseQuestionlibId="+courseQuestionlibId+"&delFlag="+delFlag,"试题导出");
		    		return false;
		    	});
    ...
    <body>
	    <ul class="nav nav-tabs">
			<li class="active"><a href="${ctx}/school/schoolversionQuestion/list?extId=${extId}&courseQuestionlibId=${courseQuestionlibId}">试题列表</a></li>
			<li><a href="${ctx}/school/schoolversionQuestion/form?extId=${extId}&courseQuestionlibId=${courseQuestionlibId}&courseVersionId=${courseVersionId}">试题添加</a></li> 
			<shiro:hasPermission name="school.web:schoolversionQuestion:download">
				<shiro:hasPermission name="school.web:schoolversionQuestion:show">
			...
			<shiro:hasPermission name="school.web:schoolversionQuestion:delete">
				<li style="margin-left: 20px;" class="btns"><input id="btnDelete" class="btn btn-primary" type="button" value="批量删除" onclick="deleteLibQuestions()" /></li>
			 </shiro:hasPermission>
		</ul>
		<form:form id="searchForm" modelAttribute="versionQuestion" action="${ctx}/school/schoolversionQuestion/list" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label>状态： </label>
			<form:radiobuttons onclick="$('#searchForm').submit();" path="delFlag" items="${fns:getDictList('question_del_flag')}" itemLabel="label" itemValue="value" htmlEscape="false" /></li>--%>
			<li class="clearfix"></li>
		</ul>
	</form:form>
    <sys:message content="${message}"/>
    <form id="formList" action="${ctx}/school/schoolversionQuestion/delete?extId=${extId}&courseQuestionlibId=${courseQuestionlibId}" method="post">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed"> 
		...
		<c:forEach items="${page.list}" var="versionQuestion" varStatus="status">
			<tr>
				...
				<td>
				   <a href="${ctx}/school/schoolversionQuestion/form?id=${versionQuestion.id}&extId=${extId}&courseQuestionlibId=${courseQuestionlibId}">
				...
				<td>
					<%-- <c:if test="${versionQuestion.delFlag eq '2'}">
						<shiro:hasPermission name="questionlib:versionQuestion:auditor">
	    					<a href="${ctx}/school/schoolversionQuestion/delete?ids=${versionQuestion.id}&delFlag=0&courseQuestionlibId=${courseQuestionlibId}&extId=${extId}"
					...
					<shiro:hasPermission name="questionlib:versionQuestion:auditor">
						<a href="${ctx}/school/schoolversionQuestion/delete?ids=${versionQuestion.id}&delFlag=2&courseQuestionlibId=${courseQuestionlibId}&extId=${extId}" 
						onclick="return confirmx('确认撤销审吗？', this.href)">撤销重审</a>
					</shiro:hasPermission>
					</c:if>--%>
    				<a href="${ctx}/school/schoolversionQuestion/form?id=${versionQuestion.id}&operation=view&extId=${extId}&courseQuestionlibId=${courseQuestionlibId}">查看</a>
    				<shiro:hasPermission name="questionlib:versionQuestion:edit"><a href="${ctx}/school/schoolversionQuestion/form?id=${versionQuestion.id}&extId=${extId}&courseQuestionlibId=${courseQuestionlibId}">修改</a>
						<shiro:hasPermission name="school.web:schoolversionQuestion:delete">
							<a id="deleteQuestion" href="${ctx}/school/schoolversionQuestion/delete?id=${versionQuestion.id}&extId=${extId}&courseQuestionlibId=${courseQuestionlibId}&delFlag=1" onclick="return confirmx('确认要删除该试题吗？', this.href)">删除</a>
						</shiro:hasPermission>
					</shiro:hasPermission>
				</td>
			</tr>
