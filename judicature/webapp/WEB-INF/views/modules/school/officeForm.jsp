...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/school/shoolOffice/list?extId=${extId}">机构列表</a></li>
		<li class="active"><a href="${ctx}/school/shoolOffice/form?extId=${extId}">机构<%-- <shiro:hasPermission name="sys:office:edit">${not empty office.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="sys:office:edit">查看</shiro:lacksPermission> --%>
			<shiro:hasPermission name="school.web:shoolOffice:edit">${not empty office.id?'修改':'添加'}</shiro:hasPermission>
		</a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="office" action="${ctx}/school/shoolOffice/save?extId=${extId}" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<span><form:input path="name" htmlEscape="false" maxlength="50" class="required"/>
