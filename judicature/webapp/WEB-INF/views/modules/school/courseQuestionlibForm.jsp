...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/school/schoolcourseQuestionlib/list">题库列表</a></li>
		<li class="active">
		   <a href="${ctx}/school/schoolcourseQuestionlib/form?id=${courseQuestionlib.id}">题库<%-- <shiro:hasPermission name="questionlib:courseQuestionlib:edit">${not empty courseQuestionlib.id?'修改':'添加'}</shiro:hasPermission> <shiro:lacksPermission name="questionlib:courseQuestionlib:edit">查看</shiro:lacksPermission>--%>
			<shiro:hasPermission name="questionlib:schoolClass:edit">${not empty courseQuestionlib.id?'修改':'添加'}</shiro:hasPermission>
			
			</a>
		</li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="courseQuestionlib" action="${ctx}/school/schoolcourseQuestionlib/save?courseVesionId=${courseVesionId} " method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<span class="help-inline"><font color="red">* (注：题库名称请按照示例填写：专业名-课程名-版本名-XXX题库 )</font> </span>
			</div>
		</div>
		<div class="control-group">
			 <label class="control-label">专业：</label>
			  ...
			  <select class="courseSelect" name="courseId" id="courseId"  style="width:200px;">
			    <option>请选择</option>
			  	<c:forEach items="${specialtyId==null?null:(questionlib:getCourseBySpecialtyId(specialtyId))}" var="course">
				   	<option value="${course.id}" ${courseId==course.id?"selected":""}  >${course.title} </option>
				 </c:forEach>
			   </select>
			 版本： 
		       <select name="versionId" id="courseVesionSelect"  class="input-medium"  style="width:200px;">
		        <option>请选择</option>
				  <c:forEach items="${courseId==null?null:(questionlib:getCourseVersionByCourseId(courseId))}" var="courseVesion">
