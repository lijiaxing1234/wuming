...
<head>
	...
	//下载导入模板
		...
		function recom(id,status){
			$.ajax({
				type : "POST", //请求方式
				url : "${ctx}/questionlib/courseKnowledge/recommend?id="+id+"&status="+status, //请求路径
		...
		function sortZT(id,sort){
			alert(id+","+sort);
			$.ajax({
				type : "POST", //请求方式
				url : "${ctx}/questionlib/courseKnowledge/sortZT?id="+id+"&sort="+sort, //请求路径
				cache : false, //(默认: true,dataType为script和jsonp时默认为false) jQuery 1.2 新功能，设置为 false 将不缓存此页面。
				dataType : "text",
				async: false,
				success : function(data) {
	...
	<table id="treeTableTpl" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${list}" var="row" varStatus="loop">
		    <tr id="${row.id}" >
				...
				<td>
				<fmt:formatDate value="${row.updateDate}" pattern="yyyy-MM-dd"/>
				</td>
				<td>
				<c:if test="${row.parent.id == '0'}">
				...
				<c:if test="${row.parent.id != '0'}">
				<a href="javascript:void(0);" onclick="recom('${row.id}','${row.recommend}')">
				<c:if test="${row.recommend == 0}">推荐</c:if>
				<c:if test="${row.recommend == 1}">取消推荐</c:if>
				</a>
				</c:if>
				</td>
				...
				<c:if test="${row.parent.id != '0'}">
				<input type="text" value="${row.sort}" onchange="sortZT('${row.id}',value)" style="width:50px;"/>
				</c:if>
				</td>
			</tr>
		</c:forEach>
